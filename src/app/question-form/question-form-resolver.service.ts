...
@Injectable()
export class QuestionFormResolverService implements Resolve<any> {
    ...
    return this.getQuestionObservable(id)
      .pipe(
        tap((question: Question) => {
          if (!question) {
            this.notificationsService.alert('This question does not exist, navigating you back the list of exams');
            this.location.back();
          } else {
            if (isTemplate) {
              delete question.id;
              question.title = question.title + '- used as a blueprint';
              question.question = question.question + '- used as a blueprint';
